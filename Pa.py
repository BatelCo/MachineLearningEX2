...
for j in range(10):
    for x1, y1 in zip(X, y):
        ...
        loss = max(0, 1 - np.dot(W[int(y)], x1) + np.dot(W[int(y_hat)], x1))
        tau = loss / (np.power(np.linalg.norm(x1, ord=2), 2) + (1 / (2 * C)))
        if y_hat != y:
            W[int(y), :] = W[int(y), :] + tau * x1
            W[int(y_hat), :] = np.subtract(W[int(y_hat), :], np.multiply(tau, x1))
    C = C * 45 / 50
...
for i in range(len(Xtest)):
    ...
    if test_y[i] != y_hat:
        error = error + 1
print("Pa err = ", float(error) / len(Xtest))
